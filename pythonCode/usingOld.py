import time
from itertools import repeat

import matplotlib.pyplot as plt
import numpy as np
import torch
from evalPol import eval3, eval5, eval9, eval17, sastre8
from sympy.series.approximants import approximants


def p(c, x):
    out = 0
    for i in range(len(c)):
        out += c[i] * x ** (2 * i + 1)
    return out


# Derivative of polynomial
def pp(c, x):
    out = 0
    for i in range(len(c)):
        out += c[i] * (2 * i + 1) * x ** (2 * i)
    return out


# Second derivative of polynomial
def ppp(c, x):
    out = 0
    for i in range(1, len(c)):
        out += c[i] * (2 * i + 1) * (2 * i) * x ** (2 * i - 1)
    return out


def odd_remez(q, l, u, tol):
    x = np.zeros(q + 2)
    f = np.ones(q + 2)
    n = q + 2

    # Calculate initial guess of points as Chebyshev points
    for i in range(n):
        x[i] = 0.5 * (l + u) + 0.5 * (u - l) * np.cos((2 * i + 1) * np.pi / (2 * n))
    x = np.array(sorted(x))
    old_E = np.inf
    E = 1000

    while np.abs(old_E - E) > tol:
        old_E = E
        A = np.zeros((n, n))
        for j in range(n):
            for i in range(n - 1):
                A[j, i] = x[j] ** (2 * i + 1)
        A[:, -1] = (-1) ** np.arange(n)
        c = np.linalg.solve(A, f)

        x_new = []
        coeffs_for_roots = derivative_coeffs(c[:-1])
        root_guess = np.roots(coeffs_for_roots)
        candidates = []
        for r in root_guess:
            if np.isreal(r):
                r = r.real
                if r > 0:
                    candidates.append(r)

        for guess in candidates:  # If they are too close we might have problems
            x_new.append(newton_pol(guess, c[:-1], tol))

        # Always include endpoints
        x_new = [l] + x_new + [u]

        # Sort for consistency
        x_new = np.array(sorted(x_new))

        if len(x_new) != n:
            raise ValueError(f"Expected {n} extremal points, got {len(x_new)}")

        x = x_new

        # Make sure all unique points were found
        if len(x_new) == len(set(x_new)):
            x = np.array(x_new)
        else:
            print("Error: Could not find all points")
            break

        E = c[-1]

    return c


def odd_remez_prev(coeffs, l, u, tol):
    q = len(coeffs[0]) - 1
    x = np.zeros(q + 2)
    f = np.ones(q + 2)
    n = q + 2

    # Calculate initial guess of points as Chebyshev points
    for i in range(n):
        x[i] = 0.5 * (l + u) + 0.5 * (u - l) * np.cos((2 * i + 1) * np.pi / (2 * n))
        f[i] = p(coeffs[1], p(coeffs[0], x[i])) - 1
    x = np.array(sorted(x))
    old_E = np.inf
    E = 1000
    iter = 0
    while np.abs(old_E - E) > tol and iter < 1000:
        iter += 1
        old_E = E
        A = np.zeros((n, n))
        for j in range(n):
            for i in range(n - 1):
                A[j, i] = x[j] ** (2 * i + 1)
        A[:, -1] = (-1) ** np.arange(n)
        c = np.linalg.solve(A, f)

        x_new = []

        root_guess = general_roots(l, u, 1000, c[:-1], coeffs)
        candidates = []
        for r in root_guess:
            if np.isreal(r):
                r = r.real
                if r > 0:
                    candidates.append(r)

        for guess in candidates:  # If they are too close we might have problems
            x_new.append(newton_pol_general(guess, c[:-1], coeffs, tol))

        # Always include endpoints
        # x_new = [l] + x_new + [u]

        # Sort for consistency
        x_new = np.array(sorted(x_new))

        if len(x_new) != n:
            raise ValueError(f"Expected {n} extremal points, got {len(x_new)}")

        x = x_new
        for i in range(len(x)):
            f[i] = p(coeffs[1], p(coeffs[0], x[i])) - 1

        # Make sure all unique points were found
        if len(x_new) == len(set(x_new)):
            x = np.array(x_new)
        else:
            print("Error: Could not find all points")
            break
        E = c[-1]
        print(x)
    print(f"E = {E}")

    return c


def derivative_coeffs(c):
    """
    Construct coefficients of p'(x) for an odd polynomial
    p(x) = sum c[i] x^(2i+1)

    Returns coefficients in descending powers for np.roots
    """
    q = len(c) - 1
    coeffs = np.zeros(2 * q + 1)

    for i, ci in enumerate(c):
        power = 2 * i
        coeffs[2 * q - power] = ci * (2 * i + 1)

    return coeffs


def general_roots(l, u, N, c, cOld):
    xs = np.linspace(l, u, N)
    vals = [(pp(c, x) - pp(cOld[0], x) * pp(cOld[1], p(cOld[0], x))) for x in xs]

    guesses = []
    guessVals = []

    for i in range(len(xs) - 1):
        if vals[i] * vals[i + 1] < 0:
            a, b = xs[i], xs[i + 1]
            guesses.append((a + b) / 2)

    maxGuesses = []
    while len(maxGuesses) < len(c) + 1:
        v = p(c, np.array(guesses)) - p(
            cOld[1], p(cOld[0], np.array(guesses))
        )  # Changed this form max x-value, and now it is bad. Why?
        maxGuesses.append(max(v))
        guesses.remove(max(guesses))

    return maxGuesses


def newton_pol_general(x, c, cOld, tol):
    err = 100.0
    while err > tol:
        x_new = x - (pp(c, x) - pp(cOld[0], x) * pp(cOld[1], p(cOld[0], x))) / (
            ppp(c, x)
            - (
                pp(cOld[0], x) ** 2 * ppp(cOld[1], p(cOld[0], x))
                + ppp(cOld[0], x) * pp(cOld[1], p(cOld[0], x))
            )
        )

        err = np.abs(x_new - x)
        x = x_new

    return x


def newton_pol(x, c, tol):
    err = 100.0
    while err > tol:
        x_new = x - pp(c, x) / ppp(c, x)

        err = np.abs(x_new - x)
        x = x_new

    return x


def plot_pol(c):
    x = np.linspace(0, 1, 100)
    plt.plot(x, p(c, x))
    plt.plot(x, 1 + 0 * x)


# Does this only best for degree 5, the cushioning and such?
# Maybe just do any degree not using this?
def get_all_coeffs_different_degrees(q_list, T, l=0.001):
    cushion = 0.02407327424182761
    cushion = 0
    u = 1
    all_coeffs = []
    eps = 1e-10

    for i in range(T):
        q = q_list[i]
        print(i)
        c = odd_remez(q, max(l, cushion * u), u, 1e-8)  # Make  more exact?
        if cushion * u > l:
            pl = p(c[:-1], l)
            pu = p(c[:-1], u)
            rescalar = 2 / (pl + pu)
            for i in range(len(c[:-1])):
                c[i] *= rescalar

        l = p(c[:-1], l)
        u = 2 - l
        all_coeffs.append(c[:-1])
    return all_coeffs


def test_approximation(q, l=0.001, useOld=False):
    if not useOld:
        T = len(q)
        coeffs17 = get_all_coeffs_different_degrees(q, T, l)

        x_plt = np.linspace(l, 1, 10000)

        x = np.linspace(l, 1, 10000)
        tot_degree = 1
        for i in range(T):
            x = p(coeffs17[i], x)
            tot_degree *= 2 * q[i] + 1
        print(f" min: {min(x)},  max: {max(x)}")

        plt.plot(
            x_plt,
            x,
            label=f"Total degree = {tot_degree}, d = {2 * np.array(q) + 1}",
        )

        plt.legend()
    else:
        T = len(q)
        coeffs17 = get_all_coeffs_different_degrees(q, T, l)

        extraCoeffs = odd_remez_prev(coeffs17, l, 1, 1e-8)

        print(f"Extra coeffs = {extraCoeffs}")

        x_plt = np.linspace(l, 1, 10000)

        x = np.linspace(l, 1, 10000)
        tot_degree = 1
        for i in range(T):
            x = p(coeffs17[i], x)
            tot_degree *= 2 * q[i] + 1
        x += (
            extraCoeffs[0] * x_plt
            + extraCoeffs[1] * x_plt**3
            + extraCoeffs[2] * x_plt**5
        )
        print(f" min: {min(x)},  max: {max(x)}")

        plt.plot(
            x_plt, x, label=f"Total degree = {tot_degree}, d = {2 * np.array(q) + 1}"
        )

        plt.legend()


def approxs():
    test_approximation([2, 2], 0.001)
    test_approximation([2, 2], 0.001, True)

    plt.show()


def main():
    # TODO: Some issues with the convergence of newton when the tol is too high, for large polynomials it does not converge.
    # TODO: Have some issues when using degree 3, since only one point it is not working correctly, change to just have exact solution when it is of degree 3.
    approxs()


if __name__ == "__main__":
    main()
