...
def bar_plot_dof(target_degree):
    ...
    combo_data = [(c, achieved_dof(c), achieved_degree(c)) for c in combos]
    ...
    plt.title(
        f"Degrees of Freedom vs Multiplication Combinations (sum = {target_degree})"
    )
    ...
    legend_elements = [
        Patch(facecolor=color_map[t], label=f"Total mults: {t}") for t in unique_totals
    ]
