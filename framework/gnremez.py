...
def roots(c, l, u, steps=10000):
    """Find polynomial interception with the constant function 1 in the interval l, u."""
...
def rootsp(c, l, u, steps=1000):
    """Find roots of derivative of polynomial."""
    xs = np.linspace(l, u, steps)
    guesses = []
    eps = 1e-12
...
def plot_pol(c, m, l, u):
    """Plot polynomial using new coefficients."""
...
def plot_og(c, l, u):
    """Plot polynomial using original coefficients"""
    x = np.linspace(l, u, 1000)
...
def approxs():
    """Find polynomial to be evaluated afficently."""
    m = 3 # d = 2^m
...
def NewPolarExpress(G: torch.Tensor, steps: int) -> torch.Tensor:
    """Polar express with degree 17 polynomials applied three times."""
    assert G.ndim >= 2
    X = G.bfloat16()
...
def pX(allc, m, X: torch.Tensor):
    """Evaluate matrix polynomial using efficient scheme."""
    n = X.shape[0]
...
def PolarExpress(G: torch.Tensor, steps: int, coeffs_list) -> torch.Tensor:
    """Polar Express from paper cited in report."""
    assert G.ndim >= 2
    X = G.float()  # for speed
    if G.size(-2) > G.size(-1):
...
def MachPolar(G: torch.Tensor) -> torch.Tensor:
    """Orthogonalize using any combination of degrees"""
    assert G.ndim >= 2
    X = G.float()  # for speed
    if G.size(-2) > G.size(-1):
        X = X.mT  # this reduces FLOPs
